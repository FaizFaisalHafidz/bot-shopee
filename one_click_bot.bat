@echo off & title Shopee Bot & color 0A & chcp 437 >nul
cls
echo.
echo ========================================
...
python --version >nul 2>&1 || (echo [ERROR] Install Python first! & pause & exit)

set /p s="[INPUT] Session ID (default 157658364): "
if "%s%"=="" set s=157658364

set /p v="[INPUT] Viewers (default 5): "  
if "%v%"=="" set v=5

set /p d="[INPUT] Delay (default 2s): "
if "%d%"=="" set d=2

echo.
echo [START] Starting %v% viewers for session %s%...
echo.
